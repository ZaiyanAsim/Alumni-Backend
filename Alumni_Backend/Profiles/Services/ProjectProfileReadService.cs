...
namespace Alumni_Portal.Profiles.Services
{
    public class ProjectProfileReadService
    {
        ...
        public async Task<MetaDataDTO?> GetHeaderAsync(int projectId, CancellationToken ct = default)
        {
            var result = await _repo.GetProjectHeaderAsync(projectId, ct);
            return result as MetaDataDTO;
        }

        public Task<List<MemberDTO>> GetMembersAsync(int projectId, CancellationToken ct = default)
            => _repo.GetMembers(projectId, ct);

        public Task<List<ProjectDocumentDto>> GetDocumentsAsync(int projectId, CancellationToken ct = default)
            => _repo.GetDocuments(projectId);

        public Task<List<ProjectResultsDTO>> GetResultsAsync(int projectId, CancellationToken ct = default)
            => _repo.GetResultsDTOs(projectId);

        public Task<List<ProjectDeliverablesDTO>> GetDeliverablesAsync(int projectId, CancellationToken ct = default)
            => _repo.GetDeliverablesDTOs(projectId);

        public Task<List<TechStackDTO>> GetTechStackAsync(int projectId, CancellationToken ct = default)
        ...
        public async Task<ProjectProfileResponseDTO> GetFullProfileAsync(int projectId, CancellationToken ct = default)
        {
            ...
            var methodologies = await GetMethodologiesAsync(projectId, ct);
